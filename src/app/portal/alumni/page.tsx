  ...
  return (
    ...
    >
      <div ref={containerRef} className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="dash-section flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-[#008751]/8 flex items-center justify-center shrink-0">
            {mockUser.photoUrl ? (
              ...
              <span className="text-base sm:text-lg font-semibold text-[#008751]">{mockUser.name.split(' ')[1]?.charAt(0) || 'A'}</span>
            )}
          </div>
          
          <div className="flex-1 min-w-0">
            <h1 className="text-base sm:text-xl font-semibold text-gray-900 tracking-[-0.025em]">Welcome, {mockUser.name}</h1>
            <p className="text-xs sm:text-sm text-gray-500 mt-0.5 truncate">{mockUser.specialization} &middot; Class of {mockUser.graduationYear}</p>
            <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
              ...
              <span className="inline-flex items-center gap-1 text-[10px] sm:text-xs text-gray-500 truncate">
        ...
        <Link
          href="/portal/alumni/profile"
          className="dash-section sm:hidden flex items-center justify-between px-3 py-2.5 rounded-xl bg-gray-50 text-xs font-medium text-gray-500 hover:text-gray-900 transition-colors duration-200"
        >
              ...
              {mentees.length > 0 ? (
                <div className="space-y-1">
                  {mentees.map((mentee, idx) => (
                    <div key={idx} className="flex items-center gap-2.5 sm:gap-3 px-2.5 py-2.5 sm:px-3 sm:py-3 rounded-lg hover:bg-white/60 transition-colors duration-200">
                      <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-[#008751]/8 flex items-center justify-center text-[#008751] font-semibold text-xs shrink-0">
                        {mentee.name.charAt(0)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs sm:text-sm font-medium text-gray-800">{mentee.name}</p>
                        <p className="text-[10px] sm:text-xs text-gray-400 mt-0.5">{mentee.level} &middot; {mentee.chapter}</p>
                      </div>
          ...
          <div className="space-y-5">
            ...
            <div className="dash-section relative overflow-hidden rounded-xl p-4 sm:p-5 bg-[#008751] text-white">
              ...
              <div className="relative">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[10px] sm:text-xs font-medium text-white/60 uppercase tracking-wider">Alumni Member</span>
                </div>
                <p className="text-xs sm:text-sm font-mono font-bold tracking-wide">{mockUser.regNo}</p>
                <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
                  <div>
                    <p className="text-[9px] sm:text-[10px] font-medium text-white/40 uppercase tracking-wider">Membership</p>
                    <p className="text-xs sm:text-sm font-semibold mt-0.5">{mockUser.membershipType}</p>
                  </div>
                  <Link 
            ...
            <div className="dash-section rounded-xl bg-[#008751]/4 p-4 sm:p-5">
              ...
              <Link
                ...
                className="block w-full py-2.5 bg-[#008751] hover:bg-[#006d42] text-white rounded-lg text-center text-xs sm:text-sm font-medium transition-colors duration-200"
              >
                Make a Contribution
              </Link>
            </div>
