    ...
    <DashboardLayout memberType="alumni" userName={mockUser.name} userRegNo={mockUser.regNo}>
      <div ref={pageRef} className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="dir-header">
          <h1 className="text-base sm:text-xl font-semibold text-gray-900 tracking-[-0.025em]">Alumni Directory</h1>
          <p className="text-gray-400 text-xs sm:text-sm mt-0.5">Connect with {alumniMembers.length} NAAKIMS alumni worldwide</p>
        </div>
        ...
        <div className="dir-filters flex flex-col lg:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-lg bg-white border border-gray-200 text-gray-900 placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-[#008751]/15 focus:border-[#008751]/40 transition-all text-xs sm:text-sm"
            />
          </div>
          <select
            value={selectedChapter}
            onChange={(e) => setSelectedChapter(e.target.value)}
            className="px-3 py-2 rounded-lg bg-white border border-gray-200 text-gray-700 appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#008751]/15 focus:border-[#008751]/40 text-xs sm:text-sm"
          >
          ...
          <button
            ...
            className={`px-3 py-2 rounded-lg border text-[11px] sm:text-[12px] font-semibold transition-all whitespace-nowrap ${
              showMentorsOnly
        ...
        <p className="text-gray-300 text-[11px] sm:text-[12px]">{filteredAlumni.length} alumni found</p>

        {/* Alumni Grid */}
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredAlumni.map((alumni) => (
            <div key={alumni.id} className="dir-card bg-white border border-gray-100 rounded-xl p-4 hover:border-gray-200 hover:shadow-sm transition-all duration-200 group">
              <div className="flex items-start gap-3 mb-3">
                <div className="w-9 h-9 rounded-xl bg-[#008751]/8 flex items-center justify-center text-[#008751] font-bold shrink-0 text-sm">
                  {alumni.name.split(' ')[1].charAt(0)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="text-gray-900 font-semibold text-xs sm:text-sm truncate">{alumni.name}</h3>
                    {alumni.isMentor && (
                  ...
                  <p className="text-gray-400 text-[10px] sm:text-[11px]">{alumni.specialization}</p>
                </div>
              </div>
              <div className="space-y-1 text-[10px] sm:text-[11px] text-gray-400">
                <div className="flex items-center gap-2">
              ...
              <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100">
                <button className="flex-1 py-1.5 rounded-lg bg-[#008751]/8 text-[#008751] text-[11px] sm:text-[12px] font-semibold hover:bg-[#008751]/12 transition-colors">
                  View Profile
                </button>
                <button className="px-2.5 py-1.5 rounded-lg bg-gray-50 text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition-all">
        ...
        {filteredAlumni.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 font-medium text-xs sm:text-sm">No alumni found</p>
            <p className="text-gray-300 text-[10px] sm:text-[11px] mt-1">Try adjusting your search or filters</p>
          </div>
        )}
      </div>
    </DashboardLayout>
